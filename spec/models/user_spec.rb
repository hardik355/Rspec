...
RSpec.describe User, type: :model do
  ...
  describe "Validation" do 
    before do 
      @user = User.create!(first_name: "Hardik", email: "hardik355@gmail.com")
      @category = Category.create(first_name: "Mobile")
      @product = Product.create!(first_name: "Iphone 11", category_id: @category.id)
      @order = Order.create(quantity: 10, user_id: @user.id, product_id: @product.id)
    end 

    it "Invalid user with same email" do
      user = User.new(first_name: "Hardik", email: "hardik355@gmail.com")
      user.valid?
